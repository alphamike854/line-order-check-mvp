-- V14B2C1
-- Historical/Round-owned read attribution uses immutable Round lineage.
-- Operational session/mutation authority remains current-route based.

CREATE OR REPLACE VIEW
  public.settlement_line_group_round_config_working_context
AS
SELECT
  cfg.round_id,
...
CREATE OR REPLACE VIEW public.session_round_line_group_code_risk_shadow AS
 WITH round_ranked AS (
         SELECT r_1.id,
            r_1.settlement_session_id,
            r_1.summary_group_id,
            r_1.round_no,
            r_1.status,
            r_1.opened_at,
            r_1.opened_by,
            ...
            r_1.created_at,
            r_1.updated_at,
            row_number() OVER (PARTITION BY r_1.settlement_session_id, r_1.summary_group_id ORDER BY r_1.round_no DESC) AS latest_rank
           FROM settlement_summary_group_rounds r_1
          WHERE r_1.status = ANY (ARRAY['OPEN'::text, 'CLOSED'::text])
        ), active_rounds AS (
         SELECT rr.id AS round_id,
          ...
          WHERE rr.latest_rank = 1 AND NOT (EXISTS ( SELECT 1
                   FROM settlement_summary_group_round_snapshots s
                  WHERE s.round_id = rr.id))
        ), code_base AS (
         SELECT ar.round_id,
            ar.round_no,
            ar.round_status,
            ...
            sum(oi.quantity::numeric * (1::numeric - cfg.reduction_pct / 100.0)) AS adjusted_total
           FROM order_items oi
             JOIN active_rounds ar ON ar.round_id = oi.summary_group_round_id AND ar.settlement_session_id = oi.settlement_session_id
             JOIN settlement_line_group_round_config cfg
              ON cfg.round_id = ar.round_id
             AND cfg.line_group_id = oi.line_group_id
          WHERE oi.settlement_session_id IS NOT NULL
          GROUP BY ar.round_id, ar.round_no, ar.round_status, oi.settlement_session_id, oi.business_date, cfg.line_group_id, cfg.line_group_name, ar.summary_group_id, cfg.reduction_pct, oi.category, oi.code
        ), missing_lineage_cut_totals AS (
         SELECT b.settlement_session_id,
            b.business_date,
            b.summary_group_id,
            i.category,
            i.code,
            sum(i.quantity)::bigint AS missing_cut_quantity
           FROM settlement_transfer_batches b
             JOIN settlement_transfer_batch_items i ON i.batch_id = b.id
          WHERE i.line_group_id IS NULL AND b.line_group_id IS NULL
          GROUP BY b.settlement_session_id, b.business_date, b.summary_group_id, i.category, i.code
        ), source_line_group_resolution AS (
         SELECT cb.settlement_session_id,
            cb.business_date,
            cb.summary_group_id,
            cb.category,
            cb.code,
            count(*)::integer AS source_line_group_count,
            min(cb.line_group_id) AS unique_line_group_id,
            sum(cb.order_total)::bigint AS source_order_total
           FROM code_base cb
          GROUP BY cb.settlement_session_id, cb.business_date, cb.summary_group_id, cb.category, cb.code
         ...
         SELECT b.settlement_session_id,
            ...
            i.quantity,
                CASE
                    WHEN i.line_group_id IS NOT NULL THEN i.line_group_id
                    WHEN b.line_group_id IS NOT NULL THEN b.line_group_id
                    WHEN sgr.source_line_group_count = 1 AND ml.missing_cut_quantity <= sgr.source_order_total THEN sgr.unique_line_group_id
                    ELSE NULL::text
                END AS resolved_line_group_id
           FROM settlement_transfer_batches b
             JOIN settlement_transfer_batch_items i ON i.batch_id = b.id
             LEFT JOIN missing_lineage_cut_totals ml ON ml.settlement_session_id = b.settlement_session_id AND ml.business_date = b.business_date AND ml.summary_group_id = b.summary_group_id AND ml.category = i.category AND ml.code = i.code
             LEFT JOIN source_line_group_resolution sgr ON sgr.settlement_session_id = b.settlement_session_id AND sgr.business_date = b.business_date AND sgr.summary_group_id = b.summary_group_id AND sgr.category = i.category AND sgr.code = i.code
        ), line_cuts AS (
         SELECT rlci.settlement_session_id,
          ...
          GROUP BY rlci.settlement_session_id, rlci.summary_group_id, rlci.resolved_line_group_id, rlci.category, rlci.code
         ...
         SELECT cb.round_id,
            ...
            pp.special_multiplier IS NOT NULL AND pp.special_multiplier > 0::numeric AND pp.max_special_codes IS NOT NULL AND pp.max_special_codes > 0 AS multiplier_configured,
            rp.id AS promotion_id,
            rp.target_scope,
                CASE
                    WHEN rp.id IS NULL THEN 100::numeric
                    WHEN rp.target_scope = 'ALL'::text THEN rp.point_factor_pct
                    WHEN rp.target_scope = 'SELECTED'::text AND (EXISTS ( SELECT 1
                       FROM settlement_round_point_promotion_line_groups plg
                      WHERE plg.promotion_id = rp.id AND plg.line_group_id = cb.line_group_id)) THEN rp.point_factor_pct
                    ELSE 100::numeric
                END::numeric(7,3) AS promotion_factor_pct,
            COALESCE(lc.confirmed_cut, 0::bigint) AS confirmed_cut
           FROM code_base cb
             LEFT JOIN settlement_point_profiles pp ON pp.settlement_session_id = cb.settlement_session_id AND pp.category = cb.category
             LEFT JOIN settlement_round_point_promotions rp ON rp.round_id = cb.round_id AND rp.category = cb.category AND rp.code = cb.code
             LEFT JOIN line_cuts lc ON lc.settlement_session_id = cb.settlement_session_id AND lc.summary_group_id = cb.summary_group_id AND lc.line_group_id = cb.line_group_id AND lc.category = cb.category AND lc.code = cb.code
        ), calculated AS (
         SELECT pr.round_id,
            pr.round_no,
            pr.round_status,
            pr.settlement_session_id,
            pr.business_date,
            pr.line_group_id,
            pr.line_group_name,
            pr.summary_group_id,
            ...
            pr.max_special_codes,
            pr.multiplier_configured,
            pr.promotion_id,
            pr.target_scope,
            pr.promotion_factor_pct,
            pr.confirmed_cut,
                CASE
                    WHEN pr.special_multiplier IS NOT NULL AND pr.special_multiplier > 0::numeric THEN pr.special_multiplier * pr.promotion_factor_pct / 100.0
                    ELSE NULL::numeric
                END AS effective_multiplier_raw,
                CASE
                    WHEN pr.special_multiplier IS NOT NULL AND pr.special_multiplier > 0::numeric THEN pr.order_total::numeric * pr.special_multiplier * pr.promotion_factor_pct / 100.0
                    ELSE NULL::numeric
                END AS point_exposure_raw,
            GREATEST(0::bigint, pr.order_total - pr.confirmed_cut) AS retained_quantity
         ...
         SELECT c.round_id,
            ...
            c.confirmed_cut,
            c.effective_multiplier_raw,
            c.point_exposure_raw,
            c.retained_quantity,
                CASE
                    WHEN c.special_multiplier IS NOT NULL AND c.special_multiplier > 0::numeric THEN c.retained_quantity::numeric * c.special_multiplier * c.promotion_factor_pct / 100.0
                    ELSE NULL::numeric
                END AS retained_point_exposure_raw,
            row_number() OVER (PARTITION BY c.round_id, c.line_group_id, c.category ORDER BY (COALESCE(round(c.point_exposure_raw, 2), 0::numeric)) DESC, c.order_total DESC, c.code) AS reserve_rank
           FROM calculated c
        )
 SELECT round_id,
    round_no,
    ...
    round(effective_multiplier_raw, 3)::numeric(12,3) AS effective_multiplier,
    round(point_exposure_raw, 2)::numeric(18,2) AS point_exposure,
    point_exposure_raw,
    confirmed_cut,
    retained_quantity,
    round(retained_point_exposure_raw, 2)::numeric(18,2) AS retained_point_exposure,
    retained_point_exposure_raw,
    reserve_rank,
    multiplier_configured AND order_total > 0 AND reserve_rank <= max_special_codes AS reserve_candidate
   FROM ranked r;
...
CREATE OR REPLACE FUNCTION public.dashboard_risk_snapshot(p_settlement_session_id uuid, p_summary_group_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
