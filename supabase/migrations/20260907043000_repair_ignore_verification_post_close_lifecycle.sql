-- Correct Human Verification IGNORE lifecycle
--
-- Repairs two issues introduced by the 20260907040000 backfill:
--
-- 1. Live Human IGNORE uses:
--      status = 'IGNORED'
--      resolution_type = 'IGNORED'
--    so those messages must leave the pending verification queue.
--
-- 2. PARSER_IGNORE_REQUIRES_HUMAN Reviews that were backfilled
--    after their Round was already CLOSED missed the normal
--    Round-close archive trigger. Archive them through the
--    existing authoritative archive function.
--
-- This migration:
--   - does not change parser semantics
--   - does not rewrite messages/order_items
--   - does not delete or resolve Review rows
--   - does not directly insert post-close archive rows
...
-- ============================================================
-- Repair CLOSED-round IGNORE Reviews created after Round close
--
-- archive_post_close_review_message() is the authoritative,
-- idempotent post-close snapshot boundary.
-- ============================================================
...
begin
  for target in
    ...
    join public.settlement_summary_group_rounds round_state
      on round_state.id =
    ...
    where
      ...
      and round_state.status =
        'CLOSED'

      and review.reason_codes @>
        '[{"code":"PARSER_IGNORE_REQUIRES_HUMAN"}]'::jsonb
