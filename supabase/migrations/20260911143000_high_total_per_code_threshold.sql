...
--   only category+code totals of at least 500,
--   largest provisional canonical per-code total first.
--
-- Read-model only.
...
as $$
  ...
  order by
    ...
    -- HIGH_TOTAL:
    -- largest provisional canonical category+code total first.
    case
      when upper(
        coalesce(
          nullif(
