-- In-round IGNORE Human Verification lifecycle hardening
--
-- Parser IGNORE remains machine interpretation state.
--
-- Human dispositions:
--   message_verifications row = Staff confirmed canonical order
--   Review RESOLVED / IGNORED = Staff explicitly confirmed not-order
--
-- Existing current-round IGNORE messages without Review evidence
-- receive an OPEN Review so Staff can inspect/correct them.
--
-- No parser grammar is changed by this migration.
...
-- ============================================================
-- Backfill currently actionable parser IGNORE messages
--
-- Scope is intentionally narrow:
--   - OPEN settlement only
--   - latest Summary Group Round only
--   - active / not unsent
--   - not already Human Verified
--   - no prior Review evidence of any status
-- ============================================================
...
join latest_round lr
  ...
  and lr.settlement_session_id =
    m.settlement_session_id

  and lr.summary_group_id =
    m.summary_group_id

left join public.message_verifications mv
  on mv.message_record_id =
    m.id
...
where
  ...
  and mv.message_record_id
    is null

  and not exists (
    select 1

    from public.review_items existing_review

    where
      existing_review.message_record_id =
        m.id
  )
