...
export default async (req) => {
  ...
  try {
    ...
    const [codeResult,categoryResult,overallResult,messagesResult,profileResult,promoResult,actualResult,warehouseLimitResult,riskBudgetResult,reviews,unsends,batchFreshResult]=await Promise.all([
      ...
      supabase.from("summary_group_risk_settings").select("summary_group_id,point_loss_tolerance,updated_at").order("summary_group_id"),
    ...
    for(const r of [codeResult,categoryResult,overallResult,messagesResult,profileResult,promoResult,actualResult,warehouseLimitResult,riskBudgetResult,batchFreshResult]) if(r.error) throw r.error;
    ...
    const distributionPlans=overallRisk.map((overall)=>{
      const plan=buildRiskDistributionPlan({
        rows:riskCodes.filter((row)=>row.summary_group_id===overall.summary_group_id),
        adjustedTotal:Number(overall.adjusted_received||0),
        pointLossTolerance:Number(overall.point_loss_tolerance||0),
      });
      return {summary_group_id:overall.summary_group_id,...plan};
    });
    ...
    const metricOverall = summaryGroupId
      ? {...overallRisk[0], transfer_required_total:Number(distributionPlans[0]?.transfer_required_total||0)}
      : {
          ...
          transfer_required_total:sum(distributionPlans,"transfer_required_total"),
    ...
    return json({
      ...
      risk_codes:riskCodes,category_risk:categoryRisk,overall_risk:overallRisk,distribution_plans:distributionPlans,point_profiles:profiles,point_promotions:promotions,warehouse_limits:warehouseLimits,actual_special_codes:actual,
    });
