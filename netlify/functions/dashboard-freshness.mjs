...
export default async(req)=>{
  ...
  try{
    ...
    const [{data:msg,error:me},{data:batches,error:be},{data:points,error:pe},{data:settings,error:se}]=await Promise.all([
      ...
      supabase.from("settings_change_events").select("changed_at").order("changed_at",{ascending:false}).limit(1),
    ]);
    if(me)throw me;if(be)throw be;if(pe)throw pe;if(se)throw se;
    const pointSignature=(points??[]).map(r=>`${r.category}${r.code}@${r.created_at}`).join(",");
    const freshness={
      message_at:msg?.[0]?.event_timestamp??null,
      transfer_at:batches?.[0]?.confirmed_at??null,
      points_signature:pointSignature,
      settings_at:settings?.[0]?.changed_at??null,
    };
    freshness.version=[session.id,freshness.message_at??"",freshness.transfer_at??"",freshness.points_signature,freshness.settings_at??""].join("|");
