...
function mapRpc(message){
  for(const code of ["RISK_STATE_STALE","TRANSFER_EXCEEDS_SAFE_CAPACITY","TRANSFER_EXCEEDS_CODE_AVAILABLE","TRANSFER_CODE_NOT_FOUND","SETTLEMENT_NOT_OPEN"]){
...
export default async(req)=>{
  ...
  try{
    ...
    const {data,error}=await supabase.rpc("confirm_risk_transfer_batch_safe",{
      ...
      p_expected_net_safe_capacity:s.net_safe_capacity,
      p_expected_confirmed_cut_total:s.confirmed_cut_total,
      p_expected_remaining_safe_capacity:s.remaining_safe_capacity,
