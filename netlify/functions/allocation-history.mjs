...
export default async(req)=>{
  ...
  try{
    ...
    let q=supabase.from("settlement_transfer_batches")
      .select("id,batch_number,distribution_run_id,summary_group_id,line_group_id,risk_pool,risk_model,destination,risk_mode,adjusted_received,risk_point_total,net_safe_capacity,safety_margin,risk_pct,point_loss_tolerance,risk_budget,excess_point_risk_before,warehouse_batch_limit,projected_point_reserve,projected_excess_point_risk,confirmed_cut_before,cut_total,confirmed_by,confirmed_at")
    ...
    if(ids.length){const r=await supabase.from("settlement_transfer_batch_items").select("batch_id,line_group_id,category,code,quantity,retained_before,effective_multiplier,retention_limit,recommended_transfer_before").in("batch_id",ids);if(r.error)throw r.error;items=r.data??[];}
